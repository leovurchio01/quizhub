...
const SUGGESTIONS = [
  { mode: "pomodoro", icon: "🍅", title: "Pomodoro", sub: "25 / 5" },
  { mode: "deepwork", icon: "🧠", title: "Deep Work", sub: "50 / 10" },
  { mode: "exam", icon: "📝", title: "Simulazione", sub: "esame" },
  { mode: "chrono", icon: "🔁", title: "Ripasso", sub: "cronometro" },
];
    ...
    <section className="dash" aria-label="Dashboard di studio">
      <div className="dash-head">
        ...
        <div className="dash-actions">
          <button className="btn primary" onClick={onUpload}>＋ Importa HTML</button>
        </div>
      </div>
      ...
      <div className="dash-grid">
        ...
        <div className="glasscard continue">
          <div className="cc-label">▶ Continua a studiare</div>
          {resume ? (
            <>
              ...
              <button className="btn primary sm" onClick={() => onOpen(resume.id)}>Riprendi</button>
            </>
          ) : (
            <>
              <div className="cc-title">Nessun file ancora</div>
              <div className="cc-meta">Importa il tuo primo HTML per iniziare.</div>
              <button className="btn primary sm" onClick={onUpload}>＋ Importa</button>
            </>
          )}
        </div>
        ...
        <div className="glasscard">
          <div className="cc-label">📊 Oggi</div>
      ...
      {recents.length > 0 && (
        <>
          <div className="cc-label sec">🕘 Recenti</div>
      ...
      {favorites.length > 0 && (
        <>
          <div className="cc-label sec">★ Preferiti</div>
          <div className="recents">
            {favorites.map((x) => (
